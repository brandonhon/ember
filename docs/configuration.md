...
| `EMBER_OLLAMA_URL` | `http://ollama:11434` | Ollama API endpoint. Used by the `ollama` backend only. Must be `http` or `https` — any other scheme fails at boot. |
| `EMBER_OLLAMA_MODEL` | `qwen2.5:0.5b` | Initial active model for the `ollama` backend. The admin UI can swap to any pulled model live. |
...
Summaries can be generated by three different transports. Ollama is the default
and is what every existing install already uses; the other two exist because the
summarizer used to speak only Ollama's native `/api/generate` and never sent an
`Authorization` header, which ruled out every hosted provider.
...
- **Model pull and delete** (`/api/admin/llm/{pull,delete}`) — a hosted provider
  has no local model cache to manage. They answer `503 not_ollama` on the other
  backends.
- **The installed-model list and the active-model picker.**
- **The host recommendation** ("this box has 8 GB, use qwen2.5:3b").
- **Temperature / Top P / Context window tuning** — these map to
  `/api/generate`'s `options` block.

Selecting a backend that cannot answer is refused: `openai` without a base URL,
`anthropic` without an API key, or a base URL that isn't `http`/`https` all
return `400`. A backend that is selected but not yet fully configured leaves
summarization switched off — incoming articles are stamped `disabled` (visible
immediately) rather than queued at a dead endpoint, and **Settings → Language
model → Summarization queue → Requeue drained articles** puts them back in line
once the configuration is finished.

> **Privacy.** The `openai` and `anthropic` backends send article title and text
> to the provider you point them at. Ember's "no article content leaves the
> host" guarantee applies to the default **Ollama** backend only. The Settings
> UI says so at the point of choice, and the API key is stored write-only — it
