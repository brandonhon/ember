...
- **`EMBER_DISABLE_SUMMARIES` is no longer a hard kill switch.** It now only seeds the boot-time default for a persisted `summaries_enabled` setting that an admin can flip at runtime in **Settings → Language model**. If you were relying on the env var alone to guarantee summaries stay off, that guarantee no longer holds after upgrading — an admin can turn summaries back on from the UI without touching `.env`. Restart with the var still set and summaries default off exactly as before; the difference is only that it's now overridable without a restart.
