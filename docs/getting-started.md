...
5. Click **Browse starter packs** or paste a feed URL — or just a domain like `example.com` (no `https://` needed) — into the sidebar "+ Add feed". Ember auto-discovers the feed; if the site publishes more than one, a picker lets you choose which to add. Recognized URL shapes:
...
- **OPML** — import your subscription list (or export Ember's at any time). Folders become categories and the feeds inside them are filed under that category; nested sub-folders flatten into their top-level folder (Ember's categories are flat). Feeds outside any folder import uncategorized. Re-importing the same file won't duplicate feeds or categories — and the count it reports is the number of subscriptions it actually *added*, so a repeat import correctly says 0 rather than re-counting everything in the file.
