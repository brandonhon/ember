...
3. Each worker calls `Fetcher.Fetch(url, etag, last_modified)`:
   ...
   - Error → increment error_count, schedule next try with exponential backoff (capped at `MaxInterval`). This covers **parse** failures as well as fetch failures: a feed that responds 200 with something gofeed can't parse is just as broken as one that won't connect, and both increment `error_count`, so both widen the retry interval. (Parse failures used to skip the backoff and keep re-requesting at the floor interval indefinitely.)
...
Only the `shared` view (explicit one-off share) and board views (explicit curation) skip the predicate. Per-feed and per-category views dedup too: a duplicated story is owned by its lowest-id (first-ingested) feed and counted/shown once. Their sidebar badges go through `CountUnreadByFeed` / `CountUnreadByCategory`, which reuse `buildArticleFilter`, so per-feed badges sum to All Unread and never exceed their column. Rows with both `cluster_id` and `title_fingerprint` empty pass unconditionally (no signal in either dimension). **Star/later/tag state is per-row** and not propagated across siblings (you can save or tag one specific copy). **Read state, however, propagates across the cluster**: because the suppressor only hides *unread* copies, marking just the visible winner read would let its hidden sibling resurface as unread on the next poll — a phantom duplicate. So every mark-read path (opening an article, the keyboard read toggle, and bulk "mark all read") routes through `MarkReadWithSiblings` (the `include_siblings` flag on `POST /api/articles/read`), which also marks the cluster's still-unread siblings read. The reverse direction is handled at ingest: a duplicate that arrives *after* you've read the story — a second feed's copy, or the same feed re-publishing under a fresh `guid`+`content_hash` (which `UpsertArticle` stores as a new row) — would otherwise come in unread with no read winner to suppress it. So `UpsertArticle` inherits the read state from an already-read cluster sibling at insert time, the ingest-side mirror of `MarkReadWithSiblings`. Between the two, reading one copy of a syndicated story means you've read the story — and its past *and* future duplicates stay read.
...
- **Optimistic badge updates are window-aware.** Marking an article read adjusts the Fresh and All Unread badges locally before the server responds, then a debounced `refreshSmartCounts` reconciles. Both badges are server-computed over a window, so each optimistic delta must apply that same window — `/api/me` reports `fresh_window_seconds` and `unread_window_seconds` for exactly this. It matters because Starred, Read Later, Shared and board lists are deliberately *unwindowed* and routinely show older articles: counting one of those toward the All-Unread delta would subtract an article the badge never included. The windows are sent as widths rather than cutoff timestamps, since the server recomputes `now - width` per request and a timestamp would go stale in a long-lived tab.
