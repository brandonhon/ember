...
Only the `shared` view (explicit one-off share) and board views (explicit curation) skip the predicate. Per-feed and per-category views dedup too: a duplicated story is owned by its lowest-id (first-ingested) feed and counted/shown once. Their sidebar badges go through `CountUnreadByFeed` / `CountUnreadByCategory`, which reuse `buildArticleFilter`, so per-feed badges sum to All Unread and never exceed their column. Rows with both `cluster_id` and `title_fingerprint` empty pass unconditionally (no signal in either dimension). Read/star/tag state is per-row and **not** propagated across siblings — opening a peer of a row you already read shows it as unread. The one deliberate exception is bulk **"mark all read"**: because the suppressor only hides *unread* copies, marking a visible winner read would otherwise let its hidden sibling resurface as unread, so `MarkReadWithSiblings` (behind the `include_siblings` flag on `POST /api/articles/read`) also marks the cluster's still-unread siblings read.
