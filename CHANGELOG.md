...
- **The container build's base images are pinned by digest too** — `node` and
  ...
  proposes the bumps as a single grouped PR, and a `check-pins` gate — local via
  `make security`, and the `Image pins` CI job — fails if a pin is ever dropped. Nothing about the published image's contents or
  behaviour changes.
