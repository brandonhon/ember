...
- **Summarize only the articles you mean to read.** Per-feed *Don't summarize*
  solved the all-or-nothing feed, but most feeds are mixed: you skim most of
  the articles and read the rest, and which is which is only clear after
  seeing the title. **Settings → Language model → Summaries → When I mark it**
  switches the server to on-demand: articles arrive readable straight away and
  get summarized when you star one, save it for later, or pin it to a board —
  the same signals that already keep an article past the retention window.
  Individual feeds can override the default in their **⋯** menu, and a board
  used for filing rather than reading can opt out of triggering summaries. On
  local inference that stops minutes of CPU going to articles nobody opens; on
  a metered or free-tier API it stops the spend and the quota going the same
  way; and either way the queue stays short enough that a summary you asked
  for is ready when you want it.

- **Summaries can be generated by any OpenAI-compatible endpoint, or by Claude,
  not just Ollama.** The summarizer only spoke Ollama's native `/api/generate`
  and never sent an `Authorization` header, which ruled out every hosted backend
  and left a translating proxy as the only workaround. **Settings → Language
  model → Backend** now offers Ollama (unchanged, still the default),
  **OpenAI-compatible** — a base URL plus an optional bearer token, which covers
  OpenAI, OpenRouter, Groq, Mistral, Gemini's compatibility endpoint, vLLM,
  llama.cpp and LiteLLM — and **Claude**. The choice is persisted and takes
  effect on the next article without a restart, and the API key is stored
  write-only (never sent back to the browser). Model pull and delete stay
  Ollama-only; they have no equivalent on a hosted backend. Article text is sent
  to the provider when you pick one of the hosted backends, so the local-only
  guarantee applies to the Ollama backend only.
...
- **The 90-second summarization timeout is now a setting.** It was hardcoded,
  which is below what a slow backend legitimately needs — CPU-only inference
  costs minutes on a long article, and hosted free tiers routinely take more
  than a minute. **Settings → Language model → Article visibility** now has a
  *Give up on a summary after* field (10–900 s, default 90), also settable at
  install with `EMBER_SUMMARY_TIMEOUT_SECONDS`.
...
- Bumped SPA build/dev tooling: Vite 8.2.0 → 8.2.1,
  ...
  the reason given under 0.9.6.
- Bumped Vite 8.2.2 → 8.3.0 (and its bundler `rolldown` 1.2.5 → 1.2.8). Dev-only
  build tooling, not shipped in the binary.
- Bumped Vitest 4.1.11 → 5.0.1, the unit-test runner for the SPA. A major
  release taken on its own rather than inside a grouped bump so any fallout
  would be attributable; in the event none was needed — the 63-test suite passes
  unchanged and no configuration moved. Dev-only, not shipped in the binary.
