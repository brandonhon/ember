...
- **Summarize only the articles you mean to read.** Per-feed *Don't summarize* solved the
  all-or-nothing feed, but most feeds are mixed: you skim most of the articles and read the rest,
  and which is which is only clear after seeing the title. **Settings → Language model → Summaries →
  When I mark it** switches the server to on-demand: articles arrive readable straight away and get
  summarized when you star one, save it for later, or pin it to a board — the same signals that
  already keep an article past the retention window. Individual feeds can override the default in
  their **⋯** menu, and a board used for filing rather than reading can opt out of triggering
  summaries. On local inference that stops minutes of CPU going to articles nobody opens; on a
  metered or free-tier API it stops the spend and the quota going the same way; and either way the
  queue stays short enough that a summary you asked for is ready when you want it.
